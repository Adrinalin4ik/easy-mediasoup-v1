...
const DEFAULT_DO_GETUSERMEDIA = true;
const DEFAULT_ENABLE_SIMULCAST = false;
...
export default class Client extends events.EventEmitter
{
	constructor(config)
	{
		...
		let url = urlFactory.getProtooUrl(config.media_server_wss, config.peerId, config.roomId);
					...
					.then(() =>
					{
						if (DO_GETUSERMEDIA) {
							return this._getLocalStream(
								{
									audio : true,
									video : (!this._webcam ? false : VIDEO_CONSTRAINS[videoResolution])
								})
								.then((stream) =>
								{
